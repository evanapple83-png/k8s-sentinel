...
import { AiNarrationError, CostCapError, RateLimitError, explainFix } from '@/lib/ai-narration';

/**
 * POST /api/ai/explain-fix — defend a choke-point's leverage claim.
 * Body: { accountId, clusterId, chokePointIndex }. Index is the 0-based
 * position into the v3 report's chokePoints[] array (the dashboard renders
...
const BodySchema = z.object({
  ...
  chokePointIndex: z.number().int().nonnegative().max(255),
});
...
export async function POST(req: Request) {
  ...
  const { accountId, clusterId, chokePointIndex } = parsed.data;
  ...
  try {
    const result = await explainFix({ accountId, userId, clusterId, chokePointIndex });
    ...
    console.error('[/api/ai/explain-fix] unexpected:', err);
