...
import { DEMO_FINDINGS } from '@/lib/mock';
...
function answer(query: string): { text: string; findings: Finding[] } {
  const q = query.toLowerCase();
  const want = (...kw: string[]) => kw.some((k) => q.includes(k));

  let matched = DEMO_FINDINGS;
  ...
  const text = filters.length
    ? `${matched.length} finding${matched.length === 1 ? '' : 's'} match ${filters.join(' + ')}. The reachable, internet-exposed payment-api dominates — it runs privileged as root and its service account can read cluster Secrets.`
